...
successfulDPathEndpoint :: DPath -> Maybe NID
...
delPath nid p = resolvePathSuccessesDetail nid p >>= mapM_ delDPath
  where
    delDPath (DPath _ xs@(_ : _) nid' []) = case lastEx xs of -- safe because list nonempty
      FromVia nid2 t -> Graph.Effect.deleteEdge @Text (Edge nid2 t nid')
    delDPath _ = pure ()
...
resolvePath' ::
  ...
  OSet DPath
resolvePath' p n g =
  nodeConsistentWithGraph g n `seq`
    fromMaybe
