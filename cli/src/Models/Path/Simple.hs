...
spec_showPath = describe "Show Path" $ do
  it "One" $
    show (One :: TPath) `shouldBe` [rq|@|]
  it "Zero" $
    show (Zero :: TPath) `shouldBe` [rq|!|]
  it "Wild" $
    show (Wild :: TPath) `shouldBe` [rq|*|]
  it "Literal" $
    show (Literal "foo" :: TPath) `shouldBe` [rq|"foo"|]
  it "RegexMatch" $
    show (RegexMatch [re|^foo.*bar$|] :: TPath) `shouldBe` [rq|re"^foo.*bar$"|]
  it "Absolute" $
    show (Absolute (smallNID 2) :: TPath) `shouldBe` [rq|@000000000002|]
  it "Backwards One" $
    show (Backwards One :: TPath) `shouldBe` [rq|~@|]
  it "Backwards Literal" $
    show (Backwards (Literal "foo") :: TPath) `shouldBe` [rq|~"foo"|]
  it "Simple sequence" $
    show (Literal "foo" :/ Literal "bar" :: TPath) `shouldBe` [rq|"foo" / "bar"|]
  it "Simple union" $
    show (Literal "foo" :+ Literal "bar" :: TPath) `shouldBe` [rq|"foo" + "bar"|]
  it "Simple intersection" $
    show (Literal "foo" :& Literal "bar" :: TPath) `shouldBe` [rq|"foo" & "bar"|]
  it "Precedence: / binds tighter than &" $
    show (Literal "a" :/ Literal "b" :& Literal "c" :: TPath) `shouldBe` [rq|"a" / "b" & "c"|]
  it "Precedence: & binds tighter than +" $
    show (Literal "a" :& Literal "b" :+ Literal "c" :: TPath) `shouldBe` [rq|"a" & "b" + "c"|]
  it "Precedence: complex expression" $
    show (Literal "a" :/ Literal "b" :& Literal "c" :+ Literal "d" :: TPath) `shouldBe` [rq|"a" / "b" & "c" + "d"|]
  it "Backwards with higher precedence" $
    show (Backwards (Literal "foo") :/ Literal "bar" :: TPath) `shouldBe` [rq|~"foo" / "bar"|]
  ...
  it "Complex with Backwards and Absolute" $
    show (Backwards (Absolute (smallNID 1)) :& Literal "test" :: TPath) `shouldBe` [rq|~@000000000001 & "test"|]
